...
export default function ColorConverter() {
  ...
  const handleInputChange = (
    ...
    try {
      ...
      if (format == "rgb") {
        ...
        if (rgbArray.length === 3 && rgbArray.every((num) => !isNaN(num))) {
          rgb = rgbArray as RGB;
      ...
      if (format == "hsl") {
        ...
        if (hslArray.length === 3 && hslArray.every((num) => !isNaN(num))) {
          hsl = hslArray as HSL;
  ...
  function setColorValue(color: string) {
	// console.log(color);
	setColors({
		rgb: colorConvert.hex.rgb(color.hex),
		hsl: colorConvert.hex.hsl(color.hex),
      ...
      <div className=" flex flex-row gap-7">
		...
		<div className=" ">
			...
			<ColorPicker
				...
				//on changes set the color from the color in the useState
